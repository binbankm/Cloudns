...
final class WorkerDetailViewModel: BaseLoadableViewModel {
    ...
    func toggleSubdomain(enabled: Bool) async {
        ...
        do {
            ...
            CloudnsToastManager.shared.showSuccess("Subdomain Updated", message: enabled ? "workers.dev enabled" : "workers.dev disabled")
        } catch {
            CloudnsToastManager.shared.showError("Failed to update subdomain", message: error.localizedDescription)
        }
