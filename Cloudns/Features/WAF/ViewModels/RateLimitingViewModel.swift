...
class RateLimitingViewModel: BaseLoadableViewModel {
    ...
    func toggleRule(zoneId: String, rule: WAFRule) async {
        ...
        do {
            ...
            CloudnsToastManager.shared.showSuccess("Rate Limit Rule", message: !rule.enabled ? "Rule enabled" : "Rule disabled")
    ...
    func createRule(zoneId: String, action: String, expression: String, description: String, enabled: Bool, ratelimit: RateLimitConfig) async {
        do {
            ...
            CloudnsToastManager.shared.showSuccess("Rate Limit Created", message: description.isEmpty ? action.uppercased() : description)
            HapticManager.notification(.success)
        } catch {
            self.errorMessage = error.localizedDescription
