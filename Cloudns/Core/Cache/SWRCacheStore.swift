...
public actor SWRCacheStore {
    ...
    public func set(_ value: some Codable & Sendable, forKey key: String, ttl: TimeInterval? = nil) {
    ...
    public func setMemoryOnly(_ value: some Codable & Sendable, forKey key: String, ttl: TimeInterval? = nil) {
        let envelope = CacheEnvelope(value: value, timestamp: Date(), ttl: ttl)
        guard let data = try? encoder.encode(envelope) else { return }
