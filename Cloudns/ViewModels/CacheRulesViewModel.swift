...
@MainActor
class CacheRulesViewModel: ObservableObject {
    let zoneId: String
    ...
    func fetchCacheRules() async {
        ...
        do {
            let rs = try await apiClient.fetchRulesetByPhase(zoneId: zoneId, phase: "http_request_cache_settings")
            self.ruleset = rs
            self.rules = rs?.rules ?? []
            self.hasFetchedData = true
        } catch {
            self.errorMessage = "Failed to load cache rules: \(error.localizedDescription)"
        }
        isLoading = false
    }
