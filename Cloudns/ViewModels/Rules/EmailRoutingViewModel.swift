...
final class EmailRoutingViewModel: BaseLoadableViewModel {
    ...
    var verifiedDestinations: [EmailDestinationAddress] {
        destinations.filter(\.isVerified)
    }
    ...
    func fetchData() async {
        await executeLoadingTask {
            ...
            async let fetchCatchAll = await (try? self.emailService.getCatchAllRule(zoneId: self.zoneId))
    ...
    func toggleEnabled(_ enabled: Bool) async {
        ...
        do {
            let updated: EmailRoutingSettings? = if enabled {
                ...
                try await emailService.disableEmailRouting(zoneId: zoneId)
            }
            if let updated {
                settings = updated
            } else {
