...
struct PagesAttachResourceBindingSheetView: View {
    ...
    private var resourceTargetPlaceholder: String {
        switch bindingType {
        case "kv": "KV Namespace ID"
        case "d1": "D1 Database ID"
        case "r2": "R2 Bucket Name"
    ...
    private func hasExistingResources(for type: String) -> Bool {
        switch type {
        case "kv": !kvNamespaces.isEmpty
        case "d1": !d1Databases.isEmpty
        case "r2": !r2Buckets.isEmpty
        default: false
        }
    }
    ...
    private func autoSelectFirstResource(for type: String) {
        ...
        case "kv":
            if let first = kvNamespaces.first {
                ...
                if variableName.isEmpty {
                    variableName = first.title.uppercased().replacingOccurrences(of: "-", with: "_")
                }
            }
        case "d1":
        ...
        case "r2":
            if let first = r2Buckets.first {
                ...
                if variableName.isEmpty {
                    variableName = "\(first.name.uppercased().replacingOccurrences(of: "-", with: "_"))_BUCKET"
                }
            }
        case "ai":
            if variableName.isEmpty {
