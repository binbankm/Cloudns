...
public struct R2Bucket: Codable, Identifiable, Equatable, Hashable, Sendable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        name = try container.decodeIfPresent(String.self, forKey: .name) ?? "Bucket"
...
public struct R2Object: Codable, Identifiable, Equatable, Sendable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        etag = (try? container.decodeIfPresent(String.self, forKey: .etag)) ?? (try? container.decodeIfPresent(String.self, forKey: .httpEtag))
        version = try? container.decodeIfPresent(String.self, forKey: .version)
        uploaded = try? container.decodeIfPresent(String.self, forKey: .uploaded)
        storageClass = (try? container.decodeIfPresent(String.self, forKey: .storageClass)) ?? "Standard"
...
public struct R2CustomDomain: Codable, Identifiable, Equatable, Sendable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        minTLS = try? container.decodeIfPresent(String.self, forKey: .minTLS)
        zoneId = (try? container.decodeIfPresent(String.self, forKey: .zoneId)) ?? (try? container.decodeIfPresent(String.self, forKey: .zone_id))
        zoneName = (try? container.decodeIfPresent(String.self, forKey: .zoneName)) ?? (try? container.decodeIfPresent(String.self, forKey: .zone_name))
        ...
        if let statusStr = try? container.decode(String.self, forKey: .status) {
            ...
            status = statusObj["ownership"] ?? statusObj["ssl"] ?? "active"
        } else {
            status = "active"
        }
    }
...
public struct R2CORSRule: Codable, Identifiable, Equatable, Sendable {
    public var id: String {
        customId ?? "\(allowedOrigins.joined(separator: ","))-\(allowedMethods.joined(separator: ","))"
    }
    ...
    public init(from decoder: Decoder) throws {
        ...
        exposeHeaders = try? container.decodeIfPresent([String].self, forKey: .exposeHeaders)
        maxAgeSeconds = try? container.decodeIfPresent(Int.self, forKey: .maxAgeSeconds)
